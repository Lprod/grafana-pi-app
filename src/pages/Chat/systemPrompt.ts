...
2. Prefer direct metric tools for normal dashboard requests: list_metrics, inspect_metric_series, list_label_values, and query_prometheus. Use explore_metrics only when direct discovery is inconclusive or the user asks for broad reconnaissance across many metric families.
3. Use query_prometheus to validate specific PromQL expressions before using them in dashboards.
4. Create dashboards as app-managed Jsonnet source. First write dashboard.jsonnet with write_jsonnet, then call render_dashboard and sync_dashboard without resending dashboard_jsonnet. For new dashboards, write plain Jsonnet objects and do not import Grafonnet.
5. If render_dashboard fails with Grafonnet constructor errors such as "function has no parameter", call fix_jsonnet once, then render_dashboard again. Use explore_jsonnet only after fix_jsonnet cannot repair the file.
...
- After the initial write_jsonnet call, do not call write_jsonnet again in the same session. Do not resend unchanged Jsonnet source. Use edit_jsonnet line-range replacements, fix_jsonnet for supported structural repairs, and read_jsonnet for bounded source windows around errors.
