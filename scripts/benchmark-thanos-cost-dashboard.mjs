...
const child = spawn(process.execPath, ['scripts/benchmark-agent.mjs'], {
  ...
  env: {
    ...
    BENCH_LABEL: process.env.BENCH_LABEL ?? 'Thanos cost dashboard benchmark',
    BENCH_LOG_PREFIX: process.env.BENCH_LOG_PREFIX ?? 'thanos-cost-dashboard-benchmark',
    BENCH_TEST_FILE: process.env.BENCH_TEST_FILE ?? 'tests/agentThanosCostDashboardBenchmark.spec.ts',
    BENCH_TIMEOUT_MS: process.env.BENCH_TIMEOUT_MS ?? '600000',
