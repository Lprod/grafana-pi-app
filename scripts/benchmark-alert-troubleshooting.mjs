...
const child = spawn(process.execPath, ['scripts/benchmark-agent.mjs'], {
  ...
  env: {
    ...
    BENCH_LABEL: process.env.BENCH_LABEL ?? 'alert troubleshooting benchmark',
    BENCH_LOG_PREFIX: process.env.BENCH_LOG_PREFIX ?? 'alert-troubleshooting-benchmark',
    BENCH_TEST_FILE: process.env.BENCH_TEST_FILE ?? 'tests/agentAlertTroubleshootingBenchmark.spec.ts',
