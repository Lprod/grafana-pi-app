...
const child = spawn(process.execPath, ['scripts/benchmark-agent.mjs'], {
  ...
  env: {
    ...
    BENCH_LABEL: process.env.BENCH_LABEL ?? 'dashboard batch live editing benchmark',
    BENCH_LOG_PREFIX: process.env.BENCH_LOG_PREFIX ?? 'dashboard-batch-editing-benchmark',
    BENCH_TEST_FILE: process.env.BENCH_TEST_FILE ?? 'tests/agentDashboardEditingBenchmark.spec.ts',
    BENCH_TEST_GREP: process.env.BENCH_TEST_GREP ?? 'adds an env variable and filters every panel on a large dashboard',
    BENCH_TIMEOUT_MS: process.env.BENCH_TIMEOUT_MS ?? '480000',
