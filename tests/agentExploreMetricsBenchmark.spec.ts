...
const DEFAULT_EXPLORE_MAX_TOOL_MS = 90_000;
...
function findExploreMetricsQualityError(events: BenchmarkEvent[]) {
  ...
  const expectations = [
    ...
    { label: 'demo route values', pattern: /\/api\/orders[\s\S]*\/render\/report|\/render\/report[\s\S]*\/api\/orders/i },
    { label: 'HTTP status values', pattern: /\b200\b[\s\S]*\b500\b|\b500\b[\s\S]*\b200\b/i },
